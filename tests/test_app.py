...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # 動画作成
        self.movie_config = MovieConfig( width=640, encode_speed="fast", output_movie_path="movie/poster_test.mp4")
        image_to_movie(self.movie_config, self.image_paths)
        self.assertTrue(os.path.exists(self.movie_config.output_movie_path), "動画ファイルが存在しません")
    ...
    def test_images_are_png(self):
        ...
        for png_path in png_paths:
            with Image.open(png_path) as img:
                self.assertEqual(img.format, "PNG", f"{png_path} は PNG 形式ではありません")
    ...
    def test_image_to_movie_creation_and_duration(self, mock_subprocess_call):
        ...
        with VideoFileClip(movie_config.output_movie_path) as clip:
            expected_duration = len(self.image_paths)
            self.assertEqual(clip.duration, expected_duration)

    
    def test_upload_movie(self):
        destination_blob_name = "poster_test.mp4"
        upload_blob(bucket_name, self.movie_config.output_movie_path, destination_blob_name)
        # アップロードされた動画の存在を確認
